				...
				<nav aria-label="Page navigation example">
					<ul id="paging" class="pagination mb-0">
						<c:if test="${pageDTO.prev }">
							<li class="page-item"><a class="page-link" href="reviewReportListForm.do?pageNum=${pageDTO.startPage-1 }">Previous</a></li>
						</c:if>
						<c:forEach var="item" begin="${pageDTO.startPage }" end="${pageDTO.endPage }">
							<c:choose>
								<c:when test="${pageDTO.cri.pageNum==item }">
									<li class="page-item active"><a class="page-link" href="reviewReportListForm.do?pageNum=${item }">${item }</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link" href="reviewReportListForm.do?pageNum=${item }">${item }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pageDTO.next }">
							<li class="page-item"><a class="page-link" href="reviewReportListForm.do?pageNum=${pageDTO.endPage+1 }">Next</a></li>
						</c:if>
					</ul>
				</nav>
			</div>
...
<script>
	...
	function makeTr(report={}){
		...
		let reviewCon = report.reviewCon;
		let reviewId = report.reviewId;
		let reportCon = report.reportCon;
		
		
		td = document.createElement('td');
		td.innerHTML = '<b>' + reviewCon + '</b>' + '<br/>' + reportCon;
		td.setAttribute('style',"white-space: normal;");
		
		tdHidden = document.createElement('td');
		tdHidden.innerText = reviewId;
		...
		btn2.addEventListener('click', reviewDelete);
		btn2.innerText = "리뷰삭제";
	...
	function reviewDelete(){
		...
		let reviewId = this.parentElement.parentElement.childNodes[1].innerText;
		console.log(tr);
		console.log(reviewId);
		
		fetch("reviewReportTripDelete.do?reviewId="+reviewId, {
			method: 'delete',
		})
	...
	function answerDelete(){
		...
		fetch("reviewReportDelete.do?reviewId="+reviewId, {
			method: 'delete',
		})
		.then(resolve => resolve.json())
		.then(result => {
	...
	fetch('reviewReportList.do?pageNum='+${pageDTO.cri.pageNum})
	.then(resolve => resolve.json())
	.then(reviewList)
