				...
				<div class="row">
					<div class="col-md-12 mb-3">
						<label for="tpId">패키지</label> <select name="tpId" id="tpId"
							...
							<c:forEach var="vo" items="${tripList }">
								<option value="${vo.tpId}" <c:choose> <c:when test = "${vo.tpId eq product.tpId }"> selected </c:when> </c:choose>>${vo.tpName}</option>
							</c:forEach>
						</select>
					</div>
					...
					<div class="col-md-12 mb-3">
						...
						<textarea class="form-control" placeholder="상품 상세 설명을 입력해 주세요."
							name="prodText" id="prodText" rows="4">${product.prodText }</textarea>
					</div>
				</div>
			</div>
			<div class="mt-3 mb-4">
				<button class="btn btn-primary mt-2 animate-up-2" type="submit">수정</button>
